...
`visa-details.html → /diensten/verblijf/voorbeeld`, `faq.html → /veelgestelde-vragen`, `blog-grid-view.html → /nieuws`,
...
Note: BINDING PRE-APPROVAL CORRECTIONS

The overall LFB-103C plan is conditionally accepted, subject to these corrections:

1. SCRIPT LIFECYCLE

Do not assume that removing a script element unregisters event listeners created by that script.

For Act 1B, navigation between all nine Liviza-backed public routes must use full-document navigation rather than TanStack SPA navigation. Normalize the Liviza HTML links to the approved Dutch URLs, but let navigation reload the document so jQuery, Swiper, Waypoints, Magnific Popup and scripts.js start from a clean browser lifecycle.

Do not use broad `$(document).off(...)` cleanup unless every removed handler is proven to belong exclusively to the Liviza public surface.

Later React-native SPA navigation may be introduced only during a separately released conversion/cleanup phase.

2. NAVIGATION VERIFICATION

Test direct loading and repeated navigation across:

/ → /over-ons → /diensten → category → service detail → /veelgestelde-vragen → /nieuws → news detail → /contact → /

Verify:

- exactly one active slider/menu behavior;

- no duplicated clicks or transitions;

- no accumulating script nodes or handlers;
...
The disabled PHP form and external Google Maps iframe may produce an intentional visual/network deviation. Record these separately and exclude only those exact unsafe surfaces from the general pixel-difference threshold. Preserve their original geometry.

4. FILE MANIFEST

Include generated `src/routeTree.gen.ts` in the proposed and final manifest if route generation changes it. Never edit it manually.

5. EVIDENCE IDENTITY

Use `docs/vz-juspol-gen/execution-evidence/LFB-103C/` consistently for this eight-page batch. Do not introduce the conflicting label `LFB-103B1`.

6. CONDITIONAL LANGUAGE

Replace “sidebar/filter widgets (if present)” and similar conditional statements with exact source-confirmed sections before execution. Every selected page must have a definitive KEEP / REPURPOSE / REMOVE mapping.

Keep all other route mappings, homepage decisions, asset-reuse rules, protected boundaries and the single combined eight-page execution batch unchanged.

Update the plan document only and return:

MULTI-PAGE LIVIZA SOURCE-PORT PLAN READY FOR DELROY APPROVAL

Do not execute the implementation yet.
